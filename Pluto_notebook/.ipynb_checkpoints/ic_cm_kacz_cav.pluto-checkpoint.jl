...
# ╔═╡ b471ff1d-216b-462a-8587-e32a5b9279a4
...
function Cimmino(A::Union{Matrix, SparseMatrixCSC}, b::Vector; itmax::Int=10000, ε::Float64= 1e-3)
	...
	A,b = normalize_matriz(A,b)
	tol = norm(b- A*xₖ)
	df = DataFrame(Solução=Vector[], Tolerância=Float64[], Iterações=Int64[])
	push!(df, [xₖ, tol, k])
 		while k <= itmax && tol> ε 
...
begin
	...
	esparsidade=0.1
	Random.seed!(11)
	spa = sprand(tamanho, tamanho, esparsidade)
	vet_spa = rand(tamanho)
end
...
# ╔═╡ c2fd99ee-2fc3-4462-a2f8-72721631b6b6
